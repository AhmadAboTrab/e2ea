...
finall.add(tottalSell);
    await FirebaseFirestore.instance
      ...
      if (value.docs.isNotEmpty) {
        value.docs.forEach((doc) {
          tottalBuy += double.parse(doc.data()['total_bill_price']);
        });
      }
    });
      ...
      if (value.docs.isNotEmpty) {
        value.docs.forEach((doc) {
         List<double> quantity=[];
           List orgprice=[];
         quantity.add(double.parse( doc.data()['quantity']));
          orgprice.add(double.parse( doc.data()['original_price']));
...
finall.add(losses);
    for (int i = 0; i < medsId.length; i++) {
      await FirebaseFirestore.instance
        ...
        if (value.docs.isNotEmpty) {
          value.docs.forEach((doc) {
            pricebought += double.parse(doc.data()['prices'] * quantitysaled[i]);
          });
        }
      });
    }
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  Future<Reports> makeYearReport(var year) async {
    double tottalSell = 0.0;
    double tottalBuy = 0.0;
    double profits = 0.0;
    double losses = 0.0;
    ...
    for (int i = 0; i < medsId.length; i++) { /// moba3een
      await FirebaseFirestore.instance
          .collection('Order-bill')
          .where('medicin_id', isEqualTo: medsId[i])
          .get()
